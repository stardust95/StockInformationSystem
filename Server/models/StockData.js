...
class StockData{
    ...
    static getLatestTradeRecords(code, callback, limit){
        var conn = getConnection()
        var sql = "SELECT time, price, volume, amount FROM tradeRecords WHERE code = ? ORDER BY time"
        if( !limit ){
            limit = 10
        }
        sql += " LIMIT " + limit
    ...
    static getBlockTradeRecords(code, callback, limit){
        var conn = getConnection()
        var sql = "SELECT * FROM tradeRecords WHERE code = ? AND volume >= 8000 ORDER BY date"
        if( !limit ){
            limit = 10
        }
        sql += " LIMIT " + limit
    ...
    static getRealtimeQuotes(code, callback){
        var conn = getConnection()
        var sql = "SELECT * FROM realtimeQuotes WHERE code = ? LIMIT 1"
    ...
    static getStockNews(code, callback, limit){
        var conn = getConnection()
        var sql = "SELECT * FROM stockNews WHERE code = ? ORDER BY date DESC"
        if( !limit ){
            limit = 10
        }
        sql += " LIMIT " + limit
    ...
    static getCompanyFinReport(code, callback){
        var conn = getConnection()
        var sql = "SELECT * FROM companyFinReport WHERE `code` = ?"
        conn.query(sql, [code], callback)
    }

    static getCompanyProfit(code, callback){
        var conn = getConnection()
        var sql = "SELECT * FROM companyProfit WHERE `code` = ? ORDER BY DATE"
        conn.query(sql, [code], callback)
    }

    static getCompanyInfo(code,callback){
        var conn = getConnection()
        var sql = "SELECT * FROM companyInfo WHERE `证券代码` = ?"
        conn.query(sql, [code], callback)
    }
