var sql = {
    ...
    getBuyList: 'select buyOrSell,orderNum,price,orderID from orders where buyOrSell=? and stockID = ? order by price desc limit ?,? ',
    getSellList: 'select buyOrSell,orderNum,price,orderID from orders where buyOrSell=? and stockID = ? order by price limit ?,?',
    getUserList: 'select buyOrSell,orderNum,price,orderID from orders where userID=? and stockID = ?',
    ...
    queryBuyOrder5:'select orderNum,price from orders where buyOrSell= "0" and stockID = ? order by price desc limit 5 ',
    querySellOrder5:'select orderNum,price from orders where buyOrSell= "1" and stockID = ? order by price asc limit 5 ',
